...
# R/utils.R
#' @import data.table
#' @importFrom stats approx pnorm qnorm rnorm runif qt rt
...
# ────────────────────────────────────────────────────────────────
# piece‑wise mean on log‑scale (Method 6 logic)
# ────────────────────────────────────────────────────────────────
#' @keywords internal
#' @noRd
.meanQ <- function(q, p) {
    keep <- q > 0 & !is.na(q) & !is.na(p) # drop zeros and NA
    if (!any(keep)) {
        ## Last‑ditch fallback: arithmetic mean on log‑scale
        return(mean(log(q[q > 0]), na.rm = TRUE))
    }
    ...
    # If only one pair survives, that quantile is the mean on log‑scale
    if (length(p) == 1L) {
        return(log(q))
    }
    ...
    K <- length(p)
    dp <- diff(p)
    mid <- 0.5 * (log(q[-K]) + log(q[-1]))
    mu <- sum(dp * mid)

    # tail corrections
    if (p[1] > 0) mu <- mu + p[1] * log(q[1])
    if (p[K] < 1) mu <- mu + (1 - p[K]) * log(q[K])
