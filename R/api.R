...
fitDn <- function(
  ...
  if ("AM88" %in% models)
    DnTable <- add(DnTable, Dn_model(Dn_AM88, PGA = PGA, ky = ky, n = NS))
  if ("YG91" %in% models)
    DnTable <- add(DnTable, Dn_model(Dn_YG91, PGA = PGA, ky = ky, n = NS))
  if ("JB07" %in% models)
    DnTable <- add(DnTable, Dn_model(Dn_JB07, PGA = PGA, AI = AI, ky = ky, n = NS))
  if ("SR08" %in% models)
    DnTable <- add(DnTable, Dn_model(Dn_SR08, PGA = PGA, AI = AI, ky = ky, n = NS))
  if ("BT07" %in% models)
    DnTable <- add(DnTable, Dn_model(Dn_BT07, Ts = Ts, Sa = Sa_10_Ts, Mw = Mw,
                                     ky = ky, n = NS))
  if (tolower(BM_model) %in% c("interface", "subduction") && "BM17" %in% models)
    DnTable <- add(DnTable, Dn_model(Dn_BM17, Ts = Ts, Sa = Sa_15_Ts, Mw = Mw,
                                     ky = ky, n = NS))
  if (tolower(BM_model) %in% c("shallow", "crustal") && "BM19" %in% models)
    DnTable <- add(DnTable, Dn_model(Dn_BM19, Ts = Ts, Sa = Sa_13_Ts,
